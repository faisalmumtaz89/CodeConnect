...
pub fn tick<D: CustodianDeps>(deps: &D) -> Result<Tick> {
    ...
    match &record.state {
        ...
        // re-proves. Re-fsync (idempotent) and exit, instead of
        ...
        LaunchState::Ready => {
            ...
            if deps.coordinator_liveness() == Liveness::Gone {
                ...
                match deps.destroy() {
                    ...
                    // ServerGone (server proven dead) is terminal too.
                    | CleanupOutcome::ServerGone => {
                        // Record a durable TEARDOWN MARKER: set the
        ...
        LaunchState::Failed { .. } => match record.cleanup {
            CleanupState::Complete => {
                // Re-prove durability before exiting: if the
...
enum CleanupProgress {
    /// The marker went down: nothing is owed and the caller's own terminal verdict
    /// stands.
    Complete,
...
fn complete_cleanup<D: CustodianDeps>(deps: &D) -> Result<CleanupProgress> {
...
/// above is a within-boot one, and the sentences describing it used to say the
/// unrelated-group risk "requires pid wraparound". Across a REBOOT it
...
fn group_warrant(identity: &ProcessIdentity, recorded_boot: BootIdentity) -> GroupWarrant {
    // The boot check. Asked FIRST, before the pid is even probed: across a boot
...
fn resolve_cleanup<D: CustodianDeps>(deps: &D, record: &LaunchRecord) -> Result<Tick> {
    match deps.destroy() {
        ...
        CleanupOutcome::Absent => {
            ...
            if late_session_still_possible(deps, record) {
                if deps.boot_changed(record) {
                    // The escape: a reboot means any late session the
        ...
        // BUT the reboot escape must fire here too: a post-reboot / no-server
        // result is `Unavailable`, so the escape checked
...
impl CustodianDeps for RealDeps {
    ...
    fn destroy(&self) -> CleanupOutcome {
        // Bind the cleanup to the PERSISTED server A: read A's
        ...
        // "late UID" case). An unreadable/unparseable record must
                    ...
                    if leads_own_group && group_has_members(child.pgid) {
                        match group_warrant(&child.identity, record.boot) {
                            GroupWarrant::Warranted => {
                                ...
                                // `group_warrant`'s boot check. The command stays
                                // gated until the nonce/KERN_PROCARGS2 provenance
                                // design lands; see [`group_warrant`].
    ...
    fn cas_failed(&self, reason: &str) -> Result<()> {
        // Bounded lock: the custodian is the SOLE cleanup
...
pub fn run_custodian(args: &[String]) -> ! {
    // Parse the charter BEFORE acking: the readiness ACK must
...
/// by the recovery sweep is the same launch's janitor continuing. Appending keeps
/// both halves of that story in one file, in order.
pub fn custodian_log_path(uid: &str) -> std::path::PathBuf {
...
pub fn spawn_custodian_through_gate(
    ...
    let ready = crate::exec_gate::launch_gated(
        crate::exec_gate::GateSpec {
            ...
            // was accounting for.
            stderr: Some(custodian_log_path(uid)),
        },
        // on_intent: fsync the spawn intent before the gate exists.
        |intent| {
        ...
        // redundant custodian, so only one is ever armed.
        |ready, intent| {
...
pub fn late_host_admission(uid: &str, nonce: &str, socket: &str) -> Result<HostAdmission> {
    ...
    // Bounded: host admission must not hang on a stopped lock holder.
    ...
    // The host's own process group, recorded with the exclusive lease. A pgid we
    // cannot read is fail-closed: refuse and clean up.
...
pub(crate) mod tests {
    ...
    fn indeterminate_absent_escapes_on_a_boot_change() {
        // The reboot escape: an indeterminate launch whose destroy is Absent stays
    ...
    fn retry_cleanup_escapes_on_a_boot_change() {
        // The reboot escape must fire on the RETRY (Unavailable)
    ...
    fn ready_teardown_records_a_completion_marker() {
        // A Ready session whose coordinator died is torn down, and the
    ...
    fn a_dead_host_is_never_read_as_a_dead_session_without_server_a() {
        // **The no-A inference is RETIRED, and this pins that.**
        //
    ...
    fn failed_pending_server_gone_completes_even_when_indeterminate() {
        // A kill that drains the server yields ServerGone.
    ...
    fn late_host_is_admitted_when_valid_and_refused_after_failure() {
        ...
        .unwrap();
        // Admission now requires a recorded, proven-live custodian.
        ...
        // bogus socket has no server, and inferring absence from a socket error is
        // forbidden, so the cleanup is a harmless `Unavailable`
    ...
    fn an_arrival_write_that_fails_parks_rather_than_destroying() {
        ...
        match late_host_admission(twin, "arrivalnonce", bogus_socket).unwrap() {
            HostAdmission::CleanupOnly { cleanup, .. } => {
                // The bogus socket has no server, and inferring absence from a
                // socket error is forbidden, so the destroy reports `Unavailable`.
    ...
    fn a_group_whose_leader_pid_is_free_is_still_killed() {
        ...
        // group, and `base_record`'s fixture boot is a fake.
    ...
    /// **A straggler after the enumeration must not settle the directory.**
    ///
    ...
    /// attempt, so `dir.join(".owner.restore")` would assert
    ...
    fn a_straggler_after_the_enumeration_leaves_the_sweep_owed_not_settled() {
        ...
        // **Existing is not the same as being a WARRANT.** A
        ...
        // The restored warrant is 0600 as a FACT, not as a
    ...
    ///
    /// **The seam fires AFTER the staging file is created.** It
    ...
    /// removed. Because the staging name is unique per attempt,
    ...
    /// **The Foreign arm tells "not ours" from "could not tell".**
    ///
    ...
    /// **A `readdir` that FAILS is not a directory that ENDED.**
    ///
