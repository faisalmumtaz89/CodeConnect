//! `cc daemon install | uninstall | status | restart` — the LaunchAgent.
//!
//! ## Why launchd at all
//!
...
fn usage() {
    eprintln!(
        "\
cc daemon — the ccd LaunchAgent

  cc daemon install [--no-takeover]   write the LaunchAgent and start it
        ...
        if !takeover {
            bail!(
                "ccd is already running as pid {} ({how}), and installing has to stop \
                 ...
                 To pick up new binaries without rewriting the plist: `cc daemon restart`.\n  \
...
fn restart() -> Result<()> {
    ...
    if !plist_path().exists() {
        bail!(
            "{} is not installed; run `cc daemon install` first",
            plist_path().display()
        );
    }
...
fn status() -> Result<()> {
    ...
    match daemon_info() {
        ...
        Some(info) if info.endpoint_port == 0 => {
            ...
            println!("managed  unknown; `cc daemon install` will take over from it");
        }
        Some(info) => {
            println!(
            ...
            println!(
                ...
                match &info.launchd_label {
                    ...
                    None => "no — started by hand; `cc daemon install` takes over".to_string(),
                }
            );
        }
        None => println!(
...
///
/// The installed prefix wins over the one next to this binary: `cc daemon
...
fn legacy_daemon_probe() -> Option<DaemonInfo> {
    ...
    Some(DaemonInfo {
        ...
        version: "unknown (predates `cc daemon`)".into(),
        protocol_version: 0,
        protocol_minor: 0,
...
<!--
  Generated by `cc daemon install`. Edit that command, not this file: a
