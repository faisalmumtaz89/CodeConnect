...
//!   No I/O, no async — unit-tested against captured Phase-0 frame shapes.
...
//! protocol, the D4 generation/epoch stamping and attach envelope, the one-use
//! response-capability fanout with the broker→ccd winner disposition, and the
//! byte-fidelity comparison harness. Clean seams are marked `SEAM:` throughout and are
...
pub use response_capability::{NoCapabilities, ResponseCapabilityRegistry};
