...
///
/// **Ships in release**, because there is one person who genuinely arrives at this
/// app with no Mac, no tailnet and no way to get either: the App Store reviewer.
/// A release build has no reachable path to any screen but pairing, and pairing
/// requires a Mac running `ccd` — so without this the entire product is
/// unreviewable, and no amount of reviewer notes substitutes for a build somebody
/// can actually open.
///
/// It does not weaken the rule it used to be gated by. Sample mode never creates a
/// pairing, never opens a socket, and is reachable only by an explicit tap; a
/// release build still has no way to be *paired* except by a person. Two rules keep this from becoming the mock that
