...
- **Push notifications are a doorbell, and only a doorbell.** With an APNs provider key configured the daemon sends real notifications; without one the sender is an inert logging stub, and `hello_ack.capabilities.push` tells the phone which it is talking to. What crosses Apple's servers is deliberately austere: the session's tmux name, a risk tier, a tool *name* (never its arguments), a blocked count, and — for approvals — the words "needs you". Nothing an agent wrote ever rides a push: notification bodies are derived from the hook's *type* alone, never its message, so no command text, no diffs and no paths leave the tailnet. The app reconnects and asks the event log what is true. A paired phone can also request one **test** notification (rate-limited per device, refused for the bootstrap token) to prove the chain end to end.
