...
CodeConnect is an iOS app that connects your iPhone to the CodeConnect daemon (`ccd`) running on **your own computer**. This policy is short because the design is short: there is no CodeConnect server, no account, and no analytics. Your data moves between your own devices, with one deliberate exception named below: a push notification passes through Apple.
